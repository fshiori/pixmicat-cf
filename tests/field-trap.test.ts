...
describe('FieldTrap', () => {
  ...
  describe('getHoneypotNames', () => {
    it('應該返回一致的 honeypot 名稱', () => {
      ...
      expect(names1.subject).toBe(names2.subject);
      expect(names1.comment).toBe(names2.comment);
      expect(names1.reply).toBe(names2.reply);
    });
    ...
    it('應該包含 3 個 honeypot 欄位', () => {
      ...
      expect(names).toHaveProperty('subject');
      expect(names).toHaveProperty('comment');
      expect(names).toHaveProperty('reply');
    });
    ...
    it('honeypot 名稱應該看起來像正常欄位', () => {
      ...
      // 應該使用誘惑性的名稱
      expect(names.name.length).toBeGreaterThan(3);
      expect(names.email.length).toBeGreaterThan(3);
      expect(names.subject.length).toBeGreaterThan(3);
      expect(names.comment.length).toBeGreaterThan(3);
      expect(names.reply.length).toBeGreaterThan(3);
    });
  });
  ...
  describe('validateHoneypot', () => {
    it('應該通過空的 honeypot 欄位', () => {
      ...
      const formData = {
        [honeypotNames.name]: '',
        [honeypotNames.email]: '',
        [honeypotNames.subject]: '',
        [honeypotNames.comment]: '',
        [honeypotNames.reply]: '',
      };
    ...
    it('應該檢測多個填充的 honeypot 欄位', () => {
      const honeypotNames = getHoneypotNames();
      const formData = {
        [honeypotNames.name]: 'bot',
        [honeypotNames.email]: 'spam',
        [honeypotNames.subject]: '',
        [honeypotNames.comment]: '',
        [honeypotNames.reply]: '',
      };
  ...
  describe('整合測試', () => {
    ...
    it('應該拒絕 spam bot 請求（填充 honeypot）', () => {
      ...
      const formData = {
        ...
        [honeypotNames.name]: 'bot filled this',
        [honeypotNames.email]: '',
        [honeypotNames.subject]: '',
        [honeypotNames.comment]: '',
        [honeypotNames.reply]: '',
