...
<link rel="stylesheet" type="text/css" href="mainstyle.css" />`,
  JSHEADER: `<script type="text/javascript">
// <![CDATA[
...
<hr class="top" />
  ...
  POSTFORM: `<form action="{$SELF}" method="post" enctype="multipart/form-data" onsubmit="return c();" id="postform_main">
<div id="postform">
<!--&IF($FORMTOP,'{$FORMTOP}','')-->
...
<script type="text/javascript">preset();</script>
  ...
  THREAD: `<div class="threadpost" id="r{$NO}">
...
{$WARN_OLD}{$WARN_BEKILL}{$WARN_ENDREPLY}{$WARN_HIDEPOST}</div>`,
  REPLY: `<div class="reply" id="r{$NO}"><div class="replywrap">
...
export function parseBlock(name: string, labels: Labels = {}): string {
  return renderTemplate(blocks[name] ?? "", labels);
}

export function renderTemplate(template: string, labels: Labels): string {
  let output = template;
  if (output.includes("<!--&TOPLINKS/-->")) {
    output = output.replaceAll("<!--&TOPLINKS/-->", renderTemplate(blocks.TOPLINKS, labels));
  }
  if (output.includes("<!--&DELFORM/-->")) {
    output = output.replaceAll("<!--&DELFORM/-->", renderTemplate(blocks.DELFORM, labels));
  }
  output = output.replace(/<!--&IF\(\$([A-Z0-9_]+),'([\s\S]*?)','([\s\S]*?)'\)-->/g, (_match, key, yes, no) => {
    const value = labels[`{$${key}}`];
    return renderTemplate(value ? yes : no, labels);
  });
  output = output.replace(/\{\$[A-Z0-9_]+\}/g, (token) => String(labels[token] ?? ""));
  return output;
}
